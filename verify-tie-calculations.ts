...
async function verifyTieCalculations(): Promise<void> {
  ...
  try {
    ...
    for (const tournament of allTournaments) {
      ...
      for (const processedResult of netProcessedResults) {
        ...
        if (dbResult.position !== processedResult.position) {
          console.log(`   ❌ NET POSITION ERROR: Player ${processedResult.playerId} - DB: ${dbResult.position}, Upload: ${processedResult.position}`);
          netErrors++;
        }
        ...
        if (pointsDiff > 0.01) {
          console.log(`   ❌ NET POINTS ERROR: Player ${processedResult.playerId} - DB: ${dbResult.points}, Upload: ${processedResult.points}`);
          netErrors++;
        }
      }
      ...
      for (const processedResult of grossProcessedResults) {
        ...
        if (dbResult.grossPosition !== processedResult.position) {
          console.log(`   ❌ GROSS POSITION ERROR: Player ${processedResult.playerId} - DB: ${dbResult.grossPosition}, Upload: ${processedResult.position}`);
          grossErrors++;
        }
        ...
        if (pointsDiff > 0.01) {
          console.log(`   ❌ GROSS POINTS ERROR: Player ${processedResult.playerId} - DB: ${dbResult.grossPoints}, Upload: ${processedResult.points}`);
          grossErrors++;
        }
      }
    ...
    if (totalVerificationErrors === 0) {
      ...
      console.log(`✅ GROSS scoring: Uses tour points configuration (regardless of tournament type)`);
