...
interface TournamentAuditResult {
  ...
  totalResults: number;
  netPointsIssues: Array<{ playerId: number; position: number; currentPoints: number; expectedPoints: number }>;
  grossPointsIssues: Array<{ playerId: number; grossPosition: number; currentGrossPoints: number; expectedGrossPoints: number }>;
  ...
  try {
    ...
    for (const tournament of allTournaments) {
      ...
      if (auditResult.zeroPointsPositions.length > 0) {
        console.log(`   ⚠️  ${auditResult.zeroPointsPositions.length} positions with zero points found`);
      }
    ...
    const totalZeroPointsIssues = auditResults.reduce((sum, r) => sum + r.zeroPointsPositions.length, 0);
    ...
    if (totalNetIssues > 0 || totalGrossIssues > 0 || totalZeroPointsIssues > 0) {
      ...
      for (const result of auditResults) {
        const hasIssues = result.netPointsIssues.length > 0 || 
        ...
        if (hasIssues) {
          ...
          if (result.netPointsIssues.length > 0) {
            ...
            result.netPointsIssues.slice(0, 3).forEach(issue => {
              console.log(`     Player ${issue.playerId} pos ${issue.position}: ${issue.currentPoints} → ${issue.expectedPoints}`);
            });
          }
          ...
          if (result.grossPointsIssues.length > 0) {
            ...
            result.grossPointsIssues.slice(0, 3).forEach(issue => {
              console.log(`     Player ${issue.playerId} gross pos ${issue.grossPosition}: ${issue.currentGrossPoints} → ${issue.expectedGrossPoints}`);
            });
          }
          ...
          if (result.zeroPointsPositions.length > 0) {
            ...
            result.zeroPointsPositions.slice(0, 3).forEach(issue => {
              console.log(`     Player ${issue.playerId} ${issue.type} pos ${issue.position}: should have points`);
            });
          }
        }
      }
