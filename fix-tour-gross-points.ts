...
async function fixTourGrossPoints() {
  ...
  try {
    ...
    const tourTournaments = await db.select().from(tournaments).where(eq(tournaments.type, 'tour'));
    ...
    for (const tournament of tourTournaments) {
      ...
      for (const result of results) {
        ...
        const correctGrossPoints = getPointsFromConfig(
          result.grossPosition || 999,
          pointsConfig.tour,
        );
        ...
        if (result.grossPoints !== correctGrossPoints) {
          ...
          updatedCount++;
          console.log(
            `   📝 Updated player ${result.playerId}: position ${result.grossPosition} → ${correctGrossPoints} tour gross points (was ${result.grossPoints})`,
          );
        } else {
          console.log(
            `   ✅ Player ${result.playerId}: position ${result.grossPosition} already has correct ${correctGrossPoints} points`,
          );
        }
      }
