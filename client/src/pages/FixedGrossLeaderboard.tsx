...
<tr key={player.player.id} className="border-b border-neutral-100">
  <td className="py-3 pl-0 pr-2 whitespace-nowrap text-sm font-medium">{player.rank}</td>
  <td className="px-2 py-3 whitespace-nowrap text-sm">
    <Link href={`/players/${player.player.id}`}>
      <a className="hover:text-primary font-medium">{player.player.name}</a>
    </Link>
  </td>
  <td className="px-2 py-3 whitespace-nowrap text-sm text-center">{player.totalEvents}</td>
