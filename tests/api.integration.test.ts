...
describe('API Integration', () => {
  ...
  it('POST /api/tournaments/manual-entry creates a tournament', async () => {
    ...
    const res = await request(app).post('/api/tournaments/manual-entry').send(payload);
  ...
  it('POST /api/upload creates new player if email not found', async () => {
    const data = [
      { Email: 'newuser@example.com', Player: 'New User', Total: 80, 'Course Handicap': 12 },
    ];
    const ws = XLSX.utils.json_to_sheet(data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
  ...
  it('POST /api/upload validates gross/net calculation', async () => {
    const data = [
      { Email: 'calc@example.com', Player: 'Calc User', Total: 75, 'Course Handicap': 8 },
    ];
    const ws = XLSX.utils.json_to_sheet(data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
