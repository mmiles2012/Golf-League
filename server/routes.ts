...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.put(
    '/api/admin/player-link-requests/:requestId/approve',
    requireRole('admin'),
    async (req: any, res) => {
      try {
  ...
  app.put(
    '/api/admin/player-link-requests/:requestId/reject',
    requireRole('admin'),
    async (req: any, res) => {
      try {
        const requestId = parseInt(req.params.requestId);
        ...
        const updatedRequest = await storage.rejectPlayerLinkRequest(
          requestId,
          reviewedBy,
          reviewMessage,
        );
  ...
  app.post('/api/leagues', async (req: Request, res: Response) => {
    try {
      ...
      if (!validation.success) {
        return res
          .status(400)
          .json({ message: 'Invalid league data', errors: validation.error.format() });
      }
  ...
  app.get('/api/leaderboard/net', async (req: Request, res: Response) => {
    try {
      ...
      if (
        page === 0 &&
        leaderboardCache.net.data &&
        now - leaderboardCache.net.timestamp < leaderboardCache.net.ttl &&
        !homeClub
      ) {
        return res.json({
          data: leaderboardCache.net.data.slice(0, limit),
          total: leaderboardCache.net.data.length,
        });
      }
  ...
  app.get('/api/leaderboard/gross', async (req: Request, res: Response) => {
    try {
      ...
      if (
        page === 0 &&
        leaderboardCache.gross.data &&
        now - leaderboardCache.gross.timestamp < leaderboardCache.gross.ttl &&
        !homeClub
      ) {
        return res.json({
          data: leaderboardCache.gross.data.slice(0, limit),
          total: leaderboardCache.gross.data.length,
        });
      }
      ...
      for (let index = 0; index < data.length; index++) {
        ...
        if (
          ...
          if (isNaN(courseHandicap)) {
            console.warn(
              `Invalid handicap for ${playerName} (row ${index + 1}): '${handicapValue}', using 0`,
            );
            courseHandicap = 0;
          }
        }
  ...
  app.post('/api/tournaments/preview', async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `Generating preview for tournament: ${name}, date: ${date}, type: ${type}, with ${results.length} player results`,
      );
      ...
      const formattedData = {
        ...
        results: results.map((result: any) => ({
          ...
          grossScore:
            result.grossScore !== null && result.grossScore !== undefined
              ? Number(result.grossScore)
              : null,
          netScore:
            result.netScore !== null && result.netScore !== undefined
              ? Number(result.netScore)
              : null,
          handicap:
            result.handicap !== null && result.handicap !== undefined
              ? Number(result.handicap)
              : null,
        })),
      };
      ...
      const isStrokeNetTournament =
        scoringType === 'StrokeNet' ||
      ...
      const allPreviewResults = [...previewResults, ...newPlayerResults].sort(
  ...
  app.post('/api/tournaments/process', async (req: Request, res: Response) => {
    try {
      ...
      if (name.toLowerCase().includes('open championship')) {
        console.log(
          'Detected Open Championship - applying special handicap handling for Stroke scoring',
        );
      ...
      const validData = validationResult.data;
      console.log(
        `Processing tournament: ${validData.name}, date: ${validData.date}, type: ${validData.type}, with ${validData.results.length} player results`,
      );
      ...
      if (isStrokeNetTournament) {
        console.log(
          'Processing as StrokeNet tournament - will use Course Handicap from the uploaded file',
        );
      } else if (isStrokeTournament) {
        console.log(
          'Processing as Stroke tournament - displaying Course Handicap in tournament view',
        );
      }
      ...
      if (
        validData.type !== 'manual' &&
        pointsConfig[validData.type as keyof typeof pointsConfig]
      ) {
        const grossTieHandler = new TieHandler({
          ...pointsConfig,
          [validData.type]: pointsConfig[validData.type as keyof typeof pointsConfig].map(
            (entry) => ({
      ...
      for (const result of processedTieResults) {
        ...
        console.log(
          `Creating result for ${result.playerName}: Position ${result.displayPosition}, Points ${result.points}`,
        );
  ...
  app.post('/api/tournaments/manual-preview', async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `Generating manual tournament preview for: ${name}, date: ${date}, type: ${type}, with ${results.length} player results`,
      );
  ...
  app.post('/api/tournaments/manual-entry-preview', async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `Generating manual entry preview for tournament: ${name}, date: ${date}, type: ${type}, with ${results.length} player results`,
      );
  ...
  app.post('/api/tournaments/manual-entry', async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `Creating manual tournament: ${name}, type: ${type}, scoring: ${scoringType || 'Not specified'}`,
      );
      ...
      for (const result of results) {
        ...
        console.log(
          `Creating result for ${result.playerName}: Position ${result.position}, Points ${result.points}`,
        );
  ...
  app.put('/api/points-config', async (req: Request, res: Response) => {
    try {
      ...
      const isValid = tournamentTypes.every(
        (type) =>
          Array.isArray(pointsConfig[type]) &&
          pointsConfig[type].every(
            (item: any) =>
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (validOptions.length === 0) {
          return res
            .status(400)
            .json({ message: 'At least one valid home club option is required' });
        }
