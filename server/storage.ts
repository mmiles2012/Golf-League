...
export interface IStorage {
  ...
  updateTournament(
    id: number,
    tournament: Partial<InsertTournament>,
...
export class MemStorage implements IStorage {
  ...
  async getPlayerResultsByPlayer(playerId: number): Promise<PlayerResult[]> {
    return Array.from(this.playerResults.values()).filter((result) => result.playerId === playerId);
  }
  ...
  // Helper methods
  private async calculatePlayerHistory(
    playerId: number,
    scoreType: 'net' | 'gross',
  ...
  private initSampleData() {
    ...
    const sampleTournaments: InsertTournament[] = [
      ...
      {
        name: 'Spring League Event',
        date: new Date('2023-04-15'),
        type: 'league',
        status: 'completed',
      },
      {
        name: 'Summer League Event',
        date: new Date('2023-06-24'),
        type: 'league',
        status: 'completed',
      },
      {
        name: 'SUPR Club Challenge',
