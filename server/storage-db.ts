...
const DEFAULT_POINTS_CONFIG = {
  ...
  tour: [
    ...
    { position: 12, points: 65 },
    { position: 13, points: 60 },
    { position: 14, points: 55 },
    { position: 15, points: 53 },
    { position: 16, points: 51 },
    { position: 17, points: 49 },
    { position: 18, points: 47 },
    { position: 19, points: 45 },
    { position: 20, points: 43 },
    { position: 21, points: 41 },
    { position: 22, points: 39 },
    { position: 23, points: 37 },
    { position: 24, points: 35.5 },
    { position: 25, points: 34 },
    { position: 26, points: 32.5 },
    { position: 27, points: 31 },
    { position: 28, points: 29.5 },
    { position: 29, points: 28 },
    { position: 30, points: 26.5 },
    { position: 31, points: 25 },
    { position: 32, points: 23.5 },
    { position: 33, points: 22 },
    { position: 34, points: 21 },
    { position: 35, points: 20 },
    { position: 36, points: 19 },
    { position: 37, points: 18 },
    { position: 38, points: 17 },
    { position: 39, points: 16 },
    { position: 40, points: 15 },
    { position: 41, points: 14 },
    { position: 42, points: 13 },
    { position: 43, points: 12 },
    { position: 44, points: 11 },
    { position: 45, points: 10.5 },
    { position: 46, points: 10 },
    { position: 47, points: 9.5 },
    { position: 48, points: 9 },
    { position: 49, points: 8.5 },
    { position: 50, points: 8 },
    { position: 51, points: 7.5 },
    { position: 52, points: 7 },
    { position: 53, points: 6.5 },
    { position: 54, points: 6 },
    { position: 55, points: 5.8 },
    { position: 56, points: 5.6 },
    { position: 57, points: 5.4 },
    { position: 58, points: 5.2 },
    { position: 59, points: 5 },
    { position: 60, points: 4.8 },
    { position: 61, points: 4.6 },
    { position: 62, points: 4.4 },
    { position: 63, points: 4.2 },
    { position: 64, points: 4 },
    { position: 65, points: 3.8 },
  ],
  league: [
    // Values from League Points List PDF
    { position: 1, points: 93.75 },
    { position: 2, points: 50 },
    { position: 3, points: 43.75 },
    { position: 4, points: 40.625 },
    { position: 5, points: 37.5 },
    { position: 6, points: 34.375 },
    { position: 7, points: 28.125 },
    { position: 8, points: 25 },
    { position: 9, points: 21.875 },
    { position: 10, points: 18.75 },
    { position: 11, points: 16.25 },
    { position: 12, points: 15 },
    { position: 13, points: 13.75 },
    { position: 14, points: 11.25 },
    { position: 15, points: 10 },
    { position: 16, points: 8.75 },
    { position: 17, points: 8.125 },
    { position: 18, points: 7.5 },
    { position: 19, points: 6.875 },
...
function formatPointsConfig() {
  ...
  Object.keys(DEFAULT_POINTS_CONFIG).forEach((type) => {
    result[type as keyof PointsConfig] = [
      ...DEFAULT_POINTS_CONFIG[type as keyof typeof DEFAULT_POINTS_CONFIG],
    ];
  });
...
export class DatabaseStorage implements IStorage {
  ...
  async updateTournament(
    id: number,
    tournamentData: Partial<InsertTournament>,
  ...
  async getPlayerResultsByPlayer(playerId: number): Promise<PlayerResult[]> {
    return db.select().from(playerResults).where(eq(playerResults.playerId, playerId));
  }

  async createPlayerResult(result: InsertPlayerResult): Promise<PlayerResult> {
    const [newResult] = await db.insert(playerResults).values(result).returning();
    return newResult;
  }

  async updatePlayerResult(
    id: number,
    result: Partial<InsertPlayerResult>,
  ...
  async deletePlayerResultsByTournament(tournamentId: number): Promise<boolean> {
    await db.delete(playerResults).where(eq(playerResults.tournamentId, tournamentId));
  ...
  async processEditTournament(data: EditTournament): Promise<Tournament | undefined> {
    ...
    for (const result of results) {
      await this.createPlayerResult({
        ...
        points:
          result.points ||
          this.leaderboardCalculator.calculatePointsForPosition(result.position, type),
  ...
  async getUserPlayerProfile(
    userId: string,
  ): Promise<{ user: User; player?: Player; linkedPlayerId?: number }> {
  ...
  async linkUserToPlayer(userId: string, playerId: number): Promise<PlayerProfile> {
    ...
    const [existing] = await db
      .select()
      .from(playerProfiles)
      .where(and(eq(playerProfiles.userId, userId), eq(playerProfiles.playerId, playerId)));

    if (existing) {
      return existing;
    }

    const [link] = await db.insert(playerProfiles).values({ userId, playerId }).returning();
    return link;
  }

  async unlinkUserFromPlayer(userId: string): Promise<boolean> {
    const result = await db.delete(playerProfiles).where(eq(playerProfiles.userId, userId));
    return result && result.rowCount ? result.rowCount > 0 : false;
  }
  ...
  async getFriendsLeaderboard(
    ...
    const allPlayers =
      scoreType === 'net' ? await this.getNetLeaderboard() : await this.getGrossLeaderboard();

    // Filter by friends (assuming friend names match player names)
    return allPlayers.filter(
      (player) =>
        user.friendsList?.includes(player.player.name) || player.player.name === user.displayName,
    );
  }

  // Player link request methods
  async createPlayerLinkRequest(
    userId: string,
    playerId: number,
    requestMessage?: string,
  ): Promise<PlayerLinkRequest> {
  ...
  async approvePlayerLinkRequest(
    requestId: number,
    reviewedBy: string,
  ...
  async updateHomeClubOptions(options: string[]): Promise<string[]> {
    try {
      ...
      // Insert new options
      if (options.length > 0) {
        await db.insert(homeClubOptionsTable).values(options.map((name) => ({ name })));
      }
