  ...
  try {
    ...
    for (const tournament of allTournaments) {
      ...
      if (resultsToUpdate.length === 0) {
        console.log(`   ✅ All ${results.length} results already have gross positions for tournament ${tournament.name}`);
        continue;
      }
      
      console.log(`   🎯 Found ${resultsToUpdate.length} results needing gross position updates out of ${results.length} total`);
    ...
    const remainingNullGrossPositions = await db
      ...
      .where(and(
        isNotNull(playerResults.grossScore),
        eq(playerResults.grossPosition, null as any)
      ));
    
    if (remainingNullGrossPositions.length === 0) {
      console.log('✅ Verification: No more null gross positions found in valid results');
    } else {
      console.log(`⚠️  Warning: Still found ${remainingNullGrossPositions.length} results with null gross positions`);
    }
